            ...
            RefIntStream.range(0, itemCnt).parallel().mapToObj(RefUtil.wrapInterface(
                (IntFunction<? extends Tensor>) dataIndex -> {
                  ...
                  if (power == 2) {
                    ...
                    NthPowerActivationLayer.nthPower(power, input == null ? null : input.addRef(), inputData,
                        gradientData, outputData);
                  }
                  if (null != input)
                    input.freeRef();
                  return output;
                  ...
                  TensorArray tensorArray = new TensorArray(RefIntStream.range(0, itemCnt).parallel()
                      .mapToObj(RefUtil.wrapInterface(
                          (IntFunction<? extends Tensor>) dataIndex -> {
                            ...
                            RefIntStream.range(0, passback.length()).forEach(
                                RefUtil.wrapInterface(i -> {
